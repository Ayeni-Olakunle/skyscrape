      ...
      <main style={{ background: '#fafaf8', minHeight: '100vh', paddingTop: 80 }}>
        ...
        <div style={{ background: '#3d5496' }} className="py-10 px-6">
          <div className="max-w-5xl mx-auto">
            <a href={`/properties/${property.id}`} className="text-white/60 text-sm hover:text-white mb-4 block transition-colors">
              ← Back to property
            </a>
            <h1 className="text-2xl font-bold text-white mb-6" style={{ fontFamily: 'Georgia, serif' }}>Complete Your Booking</h1>
                ...
                <div key={s.key} className="flex items-center">
                  <div className="flex flex-col items-center">
                    <div
                      ...
                      style={{
                        background: i <= stepIndex ? '#f9cb01' : 'rgba(255,255,255,0.15)',
                        color: i <= stepIndex ? '#3d5496' : 'rgba(255,255,255,0.5)',
                      }}
                    >
                      {i < stepIndex ? '✓' : i + 1}
                    </div>
                    <span className="text-xs mt-1 font-medium hidden sm:block"
                      style={{ color: i <= stepIndex ? '#f9cb01' : 'rgba(255,255,255,0.4)' }}>
                      {s.label}
                    </span>
                  </div>
                  {i < steps.length - 1 && (
                    <div className="w-16 sm:w-24 h-0.5 mx-2 mb-4 transition-all"
                      style={{ background: i < stepIndex ? '#f9cb01' : 'rgba(255,255,255,0.2)' }} />
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="max-w-5xl mx-auto px-6 py-10 grid lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            ...
            {step === 'details' && (
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold mb-6" style={{ fontFamily: 'Georgia, serif', color: '#3d5496' }}>Guest Details</h2>
                <div className="grid sm:grid-cols-2 gap-5">
                  {[
                    { label: 'First Name', field: 'firstName', type: 'text', placeholder: 'John' },
                    { label: 'Last Name', field: 'lastName', type: 'text', placeholder: 'Doe' },
                    { label: 'Email Address', field: 'email', type: 'email', placeholder: 'john@example.com' },
                    { label: 'Phone Number', field: 'phone', type: 'tel', placeholder: '+234 800 000 0000' },
                  ].map(({ label, field, type, placeholder }) => (
                    <div key={field}>
                      <label className="block text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">{label}</label>
                      <input
                        type={type}
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">Special Requests (optional)</label>
                    <textarea
            ...
            {step === 'payment' && (
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold mb-2" style={{ fontFamily: 'Georgia, serif', color: '#3d5496' }}>Payment Details</h2>
                <p className="text-gray-400 text-sm mb-6">Your payment is secured with 256-bit SSL encryption</p>

                {/* Card visual */}
                <div className="h-44 rounded-2xl p-6 mb-8 relative overflow-hidden"
                  style={{ background: 'linear-gradient(135deg, #3d5496, #1e2d5a)' }}>
                  <div className="absolute inset-0 opacity-10" style={{ backgroundImage: 'radial-gradient(circle at 80% 20%, #f9cb01 0%, transparent 50%)' }} />
                  <div className="relative h-full flex flex-col justify-between">
                    <div className="flex justify-between items-start">
                      <div className="text-white/60 text-xs uppercase tracking-widest">LuxStay Card</div>
                    ...
                    <div>
                      ...
                      <div className="flex justify-between items-end">
                        <div>
                          ...
                          <div className="text-white text-sm font-medium">{form.cardName || 'YOUR NAME'}</div>
                        </div>
                        <div>
                          <div className="text-white/40 text-xs uppercase">Expires</div>
                          <div className="text-white text-sm font-medium">{form.expiry || 'MM/YY'}</div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid sm:grid-cols-2 gap-5">
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">Card Number</label>
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">Cardholder Name</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">Expiry Date</label>
                  ...
                  <div>
                    <label className="block text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">CVV</label>
            ...
            {step === 'confirm' && (
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold mb-6" style={{ fontFamily: 'Georgia, serif', color: '#3d5496' }}>Review Your Booking</h2>

                {/* Guest summary */}
                <div className="mb-6">
                  <h3 className="text-sm font-bold uppercase tracking-wide text-gray-400 mb-4">Guest Information</h3>
                  ...
                  {form.specialRequests && (
                    <div className="bg-gray-50 rounded-xl px-4 py-3 mt-3">
                      <div className="text-xs text-gray-400 mb-0.5">Special Requests</div>
                      <div className="text-sm text-gray-700">{form.specialRequests}</div>
                    </div>
                  )}
                </div>
                ...
                {/* Payment summary */}
                <div className="mb-6">
                  <h3 className="text-sm font-bold uppercase tracking-wide text-gray-400 mb-4">Payment Method</h3>
                  <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-4 py-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-400">{form.cardName}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-6">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-amber-800 mb-1">Cancellation Policy</div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-6">
                <h3 className="font-bold text-base mb-1" style={{ color: '#1a1a2e' }}>{property.name}</h3>
                ...
                <div className="space-y-2 text-sm border-t border-gray-100 pt-5 mt-4">
                  ...
                  <div className="flex justify-between font-bold pt-3 border-t border-gray-100" style={{ color: '#3d5496' }}>
                    <span>Total</span>
                    <span>₦{total.toLocaleString()}</span>
                  </div>
                </div>
